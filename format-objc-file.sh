...
python2 "$DIR"/custom/LiteralSymbolSpacer.py "$1"
# The formatter gets confused by C++ inline constructors that are broken onto multiple lines
python2 "$DIR"/custom/InlineConstructorOnSingleLine.py "$1"
# Add a semicolon at the end of simple macros
python2 "$DIR"/custom/MacroSemicolonAppender.py "$1"
# Add an extra newline before @implementation and @interface
python2 "$DIR"/custom/DoubleNewlineInserter.py "$1"
...
python2 "$DIR"/custom/GenericCategoryLinebreakIndentation.py "$1"
# Fix an issue with clang-format breaking up a lone parameter onto a newline after a block literal argument.
python2 "$DIR"/custom/ParameterAfterBlockNewline.py "$1"
# Fix an issue with clang-format inserting spaces in a preprocessor macro.
python2 "$DIR"/custom/HasIncludeSpaceRemover.py "$1"
# Add a newline at the end of the file
python2 "$DIR"/custom/NewLineAtEndOfFileInserter.py "$1"
